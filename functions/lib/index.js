...
exports.onQuestionCreated = exports.stripeWebhook = exports.createCustomerPortalSession = exports.confirmCheckoutHttp = exports.debugCreateCheckoutHttp = exports.createCheckoutHttp = exports.debugCheckoutHttp = exports.debugCheckout = exports.createCheckoutSession = void 0;
...
const webhookSecret = (0, params_1.defineSecret)('STRIPE_WEBHOOK_SECRET');
const priceStarter = (0, params_1.defineSecret)('STRIPE_PRICE_STARTER');
const priceGrowth = (0, params_1.defineSecret)('STRIPE_PRICE_GROWTH');
const pricePro = (0, params_1.defineSecret)('STRIPE_PRICE_PRO');
const getStripe = () => {
    const key = stripeSecret.value();
    if (!key) {
        throw new https_1.HttpsError('failed-precondition', 'Stripe secret missing. Set STRIPE_SECRET_KEY.');
    }
...
exports.createCheckoutSession = (0, https_1.onCall)({
    ...
    try {
        ...
        if (!priceStarter.value() || !priceGrowth.value() || !pricePro.value()) {
            throw new https_1.HttpsError('failed-precondition', 'Stripe price secrets missing. Set STRIPE_PRICE_STARTER/GROWTH/PRO.');
        }
        ...
        if (!plan) {
            throw new https_1.HttpsError('invalid-argument', 'Unknown price');
        }
        const price = await stripe.prices.retrieve(priceId);
        if (price.recurring) {
            throw new https_1.HttpsError('failed-precondition', 'Price must be one-time (non-recurring). Update the price in Stripe.');
        }
        const successWithSessionId = successUrl.includes('?')
        ...
        throw new https_1.HttpsError('internal', 'Stripe error', { message });
    }
});
exports.debugCheckout = (0, https_1.onCall)({
    invoker: 'public',
    secrets: [stripeSecret, priceStarter, priceGrowth, pricePro],
}, (request) => {
    return {
...
exports.debugCheckoutHttp = (0, https_1.onRequest)({
    invoker: 'public',
    secrets: [stripeSecret, priceStarter, priceGrowth, pricePro],
}, (req, res) => {
...
exports.debugCreateCheckoutHttp = (0, https_1.onRequest)({
    invoker: 'public',
    secrets: [stripeSecret, priceStarter, priceGrowth, pricePro],
}, async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
...
exports.confirmCheckoutHttp = (0, https_1.onRequest)({
    invoker: 'public',
    secrets: [stripeSecret, priceStarter, priceGrowth, pricePro],
}, async (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    ...
    try {
        ...
        await db.runTransaction(async (tx) => {
            ...
            if (paymentSnap.exists)
                return;
            const credits = userSnap.data()?.sessionCredits || 0;
            tx.set(userRef, {
...
exports.createCustomerPortalSession = (0, https_1.onCall)({
    ...
    if (!customerId) {
        throw new https_1.HttpsError('failed-precondition', 'No Stripe customer');
    }
...
exports.onQuestionCreated = (0, firestore_2.onDocumentCreated)('questions/{questionId}', async (event) => {
    ...
    await db.collection('sessions').doc(sessionId).set({
        totalQuestions: firestore_1.FieldValue.increment(1),
    }, { merge: true });
});
